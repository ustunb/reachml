...
if os.path.exists(get_scorer_file(**settings)) and not settings["overwrite"]:
    ...
    if settings["method"] == "enumerate":
        ...
        settings["reach_db"] = ReachableSetDatabase(action_set=action_set, path=get_reachable_db_file(
...
def create_resp_df(model_type, X=None, save=True):
    ...
    resp_df = row_df.melt(var_name="feature", value_name="resp", ignore_index=False).reset_index()
