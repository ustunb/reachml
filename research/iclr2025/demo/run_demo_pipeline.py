...
def main():
    ...
    parser.add_argument(
        "--models", nargs="+", type=str, default=["logreg", "xgb"]
    )
    parser.add_argument(
    ...
    if "score" in args.stages:
        for model in args.models:
            pipeline.append(
                f"python scripts/demos/calc_resp.py --data_name={args.data_name} --action_set_name={args.action_set_name} --model_type={model} {'--overwrite' if args.overwrite else ''}"
            )
