...
cols = ["data_name", "action_set_name", "model_type", "explainer_type", "metric", "value", "n"]
...
for dataset in datasets:
    ...
    for model in models:
        ...
        dm_metric_df = pd.DataFrame.from_dict(mod_met, orient="index").reset_index().rename(columns={"index": "metric", 0: "value"})
        add_columns(dm_metric_df, data_name=dataset, action_set_name=ACTION_SET, model_type=model)

        df_lst.append(dm_metric_df)

        for exp in explainers:
            dset_model_exp_metric_f = results_dir / f"{dataset}_{ACTION_SET}_{model}_{exp}_demo_metrics.csv"
